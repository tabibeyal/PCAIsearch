...
@pytest.fixture
def share_supabase_client(monkeypatch):
...
def _post_share_and_capture(client, payload, patched_urlopen):
    r = client.post("/share", json=payload)
    posted = patched_urlopen.call_args[0][0]
    posted_body = json.loads(posted.data)
    return r, posted_body


def test_supabase_share_posts_correct_query(share_supabase_client):
    fake_response = MagicMock()
    payload = _valid_payload()
    with patch("urllib.request.urlopen", return_value=fake_response) as patched_urlopen:
        _, posted_body = _post_share_and_capture(share_supabase_client, payload, patched_urlopen)
...
def test_supabase_share_posts_correct_answer(share_supabase_client):
    ...
    assert posted_body["answer"] == payload["answer"]
...
def test_supabase_share_posts_sanitized_context(share_supabase_client):
    ...
    assert posted_body["context"] == _sanitized(payload["context"])
...
def test_supabase_share_posts_generated_id(share_supabase_client):
    ...
    payload = _valid_payload()
    with patch("urllib.request.urlopen", return_value=fake_response) as patched_urlopen:
        r, posted_body = _post_share_and_capture(share_supabase_client, payload, patched_urlopen)

    assert posted_body["id"] == r.json()["id"]


def test_supabase_share_get_returns_stored_answer(share_supabase_client):
    stored = {"query": "What is dukkha?", "answer": "Suffering.", "context": [{"id": "MN 10:1"}]}
    fake_response = MagicMock()
    fake_response.__enter__.return_value = fake_response
    fake_response.read.return_value = json.dumps([stored]).encode()

    with patch("urllib.request.urlopen", return_value=fake_response):
        r = share_supabase_client.get("/share/some-id")
