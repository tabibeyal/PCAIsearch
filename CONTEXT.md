...
## Gap detection (implemented; not yet scheduled)
...
- **Gap Detector** — daily scheduled Claude Code cloud agent that scans `feedback` for new retrieval-gap candidates (`rating='down'`, qualifying category, `gap_issue_url IS NULL`), re-runs the query live through `SearchPipeline.search()` for diagnostic context (current top candidates), and files a `needs-triage` GitHub issue per candidate — capped per run, and deduped by checking for an existing open issue on the same query text first (comments instead of duplicating). Requires `NVIDIA_API_KEY` / `QDRANT_URL` / `QDRANT_API_KEY` and Supabase credentials in the agent's environment (not yet provisioned).
