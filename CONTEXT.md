...
- **AnswerComposer** — the deep module behind both `/synthesize` and `/stream`; owns the compose flow (kept context → synthesize → Guardrail → attach passages/titles → Receipt) so it's written once instead of drifting between the two routes. Depends on `Pipeline`, `Guardrail`, `PassageStore`, `SuttaTitleIndex`, and the share-receipt secret — each passed in directly, not reached-through via another collaborator. Raises on failure rather than swallowing; `answer_stream` yields typed `status`/`chunk`/`done`/`error` events, with SSE string encoding left to the route as pure transport glue.
