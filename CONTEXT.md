...
- **GROQ_API_KEY** — environment variable for LLM inference (query expansion + synthesis, both on `qwen/qwen3.8-27b` since the 2026-09-21 swap off NVIDIA; the model #212 picked, `llama-3.1-8b-instant`, is not on the account). Set as an App Platform secret. `SearchPipeline.warmup()` makes one throwaway call per model and warns, without failing to boot, if either model has been retired. NOTE: the free tier allows 8,000 input tokens/minute, and the expansion prompt alone is 2,626 — roughly two searches a minute, with the SDK silently retrying 429s beyond that.
